...
/**
 * Thrown when an internal Chronicle tool invocation exceeds its configured read timeout or
 * otherwise fails at the transport layer. Distinguishes a hung/timed-out tool from a tool that
 * returned a non-2xx HTTP response (which is reported via the response status itself), so callers
 * can present a clear "the tool didn't respond in time" message to the user instead of a generic
 * stack trace.
 */
