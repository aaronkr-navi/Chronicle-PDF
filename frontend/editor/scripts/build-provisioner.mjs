...
const destExe = join(wixDir, "chronicle-provision.exe");
...
const thumbDll = join(
  ...
  "target",
  "release",
  "chronicle_thumbnail_handler.dll",
);
if (!existsSync(thumbDll)) {
  throw new Error(`Thumbnail handler DLL not found at ${thumbDll}`);
}

copyFileSync(thumbDll, join(wixDir, "chronicle_thumbnail_handler.dll"));
