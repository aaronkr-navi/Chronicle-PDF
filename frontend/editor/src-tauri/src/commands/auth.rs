...
const KEYRING_SERVICE: &str = "chronicle-pdf";
...
fn get_keyring_entry() -> Result<Entry, String> {
    if std::env::var("CHRONICLE_PDF_TEST_FORCE_AUTH_KEYRING_FAIL").is_ok() {
...
pub fn get_refresh_token_keyring_entry() -> Result<Entry, String> {
    if std::env::var("CHRONICLE_PDF_TEST_FORCE_REFRESH_KEYRING_FAIL").is_ok() {
        return Err("Forced keyring failure for tests".to_string());
    }
...
pub async fn login(
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent("ChroniclePDF-Desktop/1.0 Tauri")
        .build()
        .map_err(|e| {
                ...
                if error_lower.contains("peer is incompatible") ||
                   ...
                   (error_lower.contains("handshake") && (error_lower.contains("tls") || error_lower.contains("ssl"))) {
                    format!(
                        ...
                        Please upgrade your server to use TLS 1.2 or higher, or use the web version of Chronicle PDF instead. \
                        Technical details: {}", e
                    )
