...
const PROVISIONING_FILE_NAME: &str = "chronicle-provisioning.json";
...
mod tests {
    ...
    fn user_app_data_provisioning_does_not_lock_ui() {
        ...
        let user_path = PathBuf::from(
            "C:\\Users\\alice\\AppData\\Roaming\\Chronicle-PDF\\chronicle-provisioning.json",
        );
        let system_dir = PathBuf::from("C:\\ProgramData\\Chronicle-PDF");
    ...
    fn system_provisioning_dir_does_lock_ui() {
        // A provisioning file in ProgramData\Chronicle-PDF (or /Library, /etc)
        ...
        let system_path = PathBuf::from(
            "C:\\ProgramData\\Chronicle-PDF\\chronicle-provisioning.json",
        );
        let system_dir = PathBuf::from("C:\\ProgramData\\Chronicle-PDF");
    ...
    fn linux_etc_provisioning_does_lock_ui() {
        let system_path = PathBuf::from("/etc/chronicle-pdf/chronicle-provisioning.json");
        let system_dir = PathBuf::from("/etc/chronicle-pdf");
        assert!(provisioning_path_is_admin_owned(
            &system_path,
            Some(&system_dir),
        ));
    ...
    fn linux_home_config_does_not_lock_ui() {
        let user_path =
            PathBuf::from("/home/alice/.config/Chronicle-PDF/chronicle-provisioning.json");
        let system_dir = PathBuf::from("/etc/chronicle-pdf");
        assert!(!provisioning_path_is_admin_owned(
            &user_path,
            Some(&system_dir),
    ...
    fn macos_library_provisioning_does_lock_ui() {
        let system_path = PathBuf::from(
            "/Library/Application Support/Chronicle-PDF/chronicle-provisioning.json",
        );
        let system_dir =
            PathBuf::from("/Library/Application Support/Chronicle-PDF");
        assert!(provisioning_path_is_admin_owned(
            &system_path,
            Some(&system_dir),
        ));
    ...
    fn macos_user_library_does_not_lock_ui() {
        let user_path = PathBuf::from(
            "/Users/alice/Library/Application Support/Chronicle-PDF/chronicle-provisioning.json",
        );
        let system_dir =
            PathBuf::from("/Library/Application Support/Chronicle-PDF");
        assert!(!provisioning_path_is_admin_owned(
    ...
    fn no_system_dir_means_no_lock() {
        ...
        // unlocked.
        let user_path = PathBuf::from("/home/alice/.config/Chronicle-PDF/chronicle-provisioning.json");
