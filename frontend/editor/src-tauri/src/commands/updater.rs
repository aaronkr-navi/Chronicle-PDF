...
/// a per-machine install writes to `C:\Program Files\Chronicle-PDF`. If the
...
/// `current_exe().parent()` on macOS is `Chronicle-PDF.app/Contents/MacOS/`
...
pub fn can_install_updates() -> CanInstallResult {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let probe = parent.join(".chronicle-auto-update-probe.tmp");
