...
const SESSION_KEY = "chronicle.filesPage.returnRoute";
const SESSION_LABEL_KEY = "chronicle.filesPage.returnLabel";
...
export function setFilesPageReturnRoute(route: string, label?: string): void {
  try {
    ...
    refreshSnapshot();
    window.dispatchEvent(new CustomEvent("chronicle-filespage-return-changed"));
  } catch {
    /* ignore */
  }
}
...
export function subscribeFilesPageReturnRoute(
  ...
  window.addEventListener("chronicle-filespage-return-changed", handler);
  return () => {
    window.removeEventListener("storage", handler);
    window.removeEventListener("chronicle-filespage-return-changed", handler);
  };
}
