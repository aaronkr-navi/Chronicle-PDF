...
export function usePageDocument(): PageDocumentHook {
  ...
  const activeFileIds = useMemo(() => {
    return allFileIds.filter((id) => {
      const stub = selectors.getChronicleFileStub(id);
  ...
  const primaryChronicleFileStub = primaryFileId
    ? selectors.getChronicleFileStub(primaryFileId)
    : null;
  const processedFilePages = primaryChronicleFileStub?.processedFile?.pages;
  const processedFileTotalPages =
    primaryChronicleFileStub?.processedFile?.totalPages;
  ...
  useEffect(() => {
    ...
    if (primaryChronicleFileStub?.processedFile) {
      placeholderDocumentRef.current = null;
      setPlaceholderVersion((v) => v + 1);
      return;
    }
      ...
      try {
        ...
        if (!canceled) {
          placeholderDocumentRef.current = {
            ...
            name:
              selectors.getChronicleFileStub(primaryFileId)?.name ?? file.name,
            file,
            pages,
            totalPages,
          };
  ...
  }, [primaryFileId, primaryChronicleFileStub?.processedFile, selectors]);
  ...
  const mergedPdfDocument = useMemo((): PDFDocument | null => {
    ...
    if (
      !primaryChronicleFileStub?.processedFile &&
      placeholderDocumentRef.current
    ) {
    ...
    const name =
      namingFileIds.length <= 1
        ? namingFileIds[0]
          ? (selectors.getChronicleFileStub(namingFileIds[0])?.name ??
            "document.pdf")
          : "document.pdf"
        : namingFileIds
            .map((id) =>
              (selectors.getChronicleFileStub(id)?.name ?? "file").replace(
                /\.pdf$/i,
                "",
              ),
            )
    ...
    const createPagesFromFile = (
      ...
      const ChronicleFileStub = selectors.getChronicleFileStub(fileId);
      if (!ChronicleFileStub) {
        return [];
      }
      ...
      const processedFile = ChronicleFileStub.processedFile;
