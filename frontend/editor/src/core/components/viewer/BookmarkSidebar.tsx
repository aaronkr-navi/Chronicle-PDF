...
import { isChronicleFile, type FileId } from "@app/types/fileContext";
...
export const BookmarkSidebar = ({
  ...
  const handleSubmitAddBookmark = useCallback(async () => {
    ...
    const resolvedFile = activeFileId
      ? allFiles.find((f) => isChronicleFile(f) && f.fileId === activeFileId)
      : (allFiles[activeFileIndex] ?? allFiles[0]);
    const resolvedFileId =
      resolvedFile && isChronicleFile(resolvedFile)
    ...
    const parentStub = selectors.getChronicleFileStub(fileId);
    if (!file || !parentStub) {
      handleFallbackToTool();
      return;
    }
