...
interface IndexedDBContextValue {
  ...
  loadMetadata: (fileId: FileId) => Promise<ChronicleFileStub | null>;
  deleteFile: (fileId: FileId) => Promise<void>;

  // Batch operations
  loadAllMetadata: () => Promise<ChronicleFileStub[]>;
  loadLeafMetadata: () => Promise<ChronicleFileStub[]>; // Only leaf files for recent files list
...
export function IndexedDBProvider({ children }: IndexedDBProviderProps) {
  ...
  const saveFile = useCallback(
    async (
      ...
      const storedFile = await fileStorage.getChronicleFileStub(fileId);

      // Cache the file object for immediate reuse
      fileCache.current.set(fileId, { file, lastAccessed: Date.now() });
      evictLRUEntries();

      // Return ChronicleFileStub from the stored file (no conversion needed)
      if (!storedFile) {
        throw new Error(
  ...
  const loadFile = useCallback(
    async (fileId: FileId): Promise<File | null> => {
      ...
      // Load from IndexedDB
      const storedFile = await fileStorage.getChronicleFile(fileId);
      if (!storedFile) return null;

      // ChronicleFile is already a File object, no reconstruction needed
  ...
  const loadMetadata = useCallback(
    async (fileId: FileId): Promise<ChronicleFileStub | null> => {
      // Load stub directly from storage service
      return await fileStorage.getChronicleFileStub(fileId);
    },
    [],
  );
  ...
  const deleteFile = useCallback(
    async (fileId: FileId): Promise<void> => {
      ...
      await fileStorage.deleteChronicleFile(fileId);
      bumpRevision();
    },
    [bumpRevision],
  );
  ...
  const loadLeafMetadata = useCallback(async (): Promise<
    ChronicleFileStub[]
  > => {
    const metadata = await fileStorage.getLeafChronicleFileStubs(); // Only get leaf files

    // All files are already ChronicleFileStub objects, no processing needed
    return metadata;
  }, []);

  const loadAllMetadata = useCallback(async (): Promise<ChronicleFileStub[]> => {
    const metadata = await fileStorage.getAllChronicleFileStubs();
  ...
  const deleteMultiple = useCallback(
    async (fileIds: FileId[]): Promise<void> => {
      ...
      await fileStorage.deleteMultipleChronicleFiles(fileIds);
      bumpRevision();
    },
    [bumpRevision],
  );
