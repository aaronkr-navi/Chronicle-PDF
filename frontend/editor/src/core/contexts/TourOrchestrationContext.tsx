...
export const TourOrchestrationProvider: React.FC<{
  ...
  const savedFilesRef = useRef<ChronicleFile[]>([]);

  // Keep a ref to always have the latest files
  const filesRef = useRef<ChronicleFile[]>(files);
  React.useEffect(() => {
  ...
  const restoreWorkbenchState = useCallback(async () => {
      ...
      try {
        await Promise.all(
          currentFiles.map((file) =>
            fileStorage.deleteChronicleFile(file.fileId),
          ),
        );
    ...
    if (savedFilesRef.current.length > 0) {
      // Create fresh File objects from ChronicleFile to avoid ID conflicts
