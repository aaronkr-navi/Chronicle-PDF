...
describe("injectOg (build-time prerender)", () => {
  ...
  it("replaces title + description and injects exactly one of each", () => {
    ...
    expect(out).toContain("<title>Compress - Chronicle PDF</title>");
  ...
  it("uses ogTitle for the social card but title for the <title> tag", () => {
    const out = injectOg(
      TEMPLATE,
      {
        ...
        title: "Chronicle - Edit any PDF. Govern every PDF.",
    ...
    expect(out).toContain(
      "<title>Chronicle - Edit any PDF. Govern every PDF.</title>",
    );
    expect(out).toContain(
  ...
  it("writes a flat file per single-segment route and a nested file (absolute base) per sub-route", async () => {
    ...
    const manifest = {
      ...
      byTool: {
        ...
        "/settings/people": {
          image: "/og_images/home.png",
          title: "People Settings - Chronicle PDF",
          description: "p",
        },
      },
      byPath: {
    ...
    expect(nested).toContain("<title>People Settings - Chronicle PDF</title>");
