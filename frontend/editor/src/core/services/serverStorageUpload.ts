...
export async function uploadHistoryChain(
...
): Promise<{ remoteId: number; updatedAt: number; chain: ChronicleFileStub[] }> {
  ...
  const finalFile = await fileStorage.getChronicleFile(finalStub.id);
  if (!finalFile) {
...
export async function uploadHistoryChains(
  ...
  const chainMap = new Map<FileId, ChronicleFileStub[]>();
  const combinedChain: ChronicleFileStub[] = [];
  ...
  if (leafStubs.length === 1) {
    const finalFile = await fileStorage.getChronicleFile(leafStubs[0].id);
    if (!finalFile) {
      throw new Error("Missing final file data for sharing.");
    }
    shareFile = finalFile;
