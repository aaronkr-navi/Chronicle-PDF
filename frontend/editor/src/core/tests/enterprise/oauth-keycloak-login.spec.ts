 ...
 *   - Keycloak on http://localhost:9080 with realm `chronicle-oauth`
 *   - Chronicle PDF on http://localhost:8080 with PREMIUM_KEY set
 *
 * Validates:
...
test.describe("Enterprise OAuth (Keycloak) — full SSO flow", () => {
  ...
  test("SSO redirect, identity in settings, real merge tool run", async ({
    ...
    await page.waitForURL(/\/realms\/chronicle-oauth\/protocol\/openid-connect/);
    await page.locator("#username").fill("oauthuser@example.com");
    await page.locator("#password").fill("oauthpassword");
    await page.locator('input[type="submit"], button[type="submit"]').click();

    // Back on Chronicle PDF, authenticated dashboard renders
