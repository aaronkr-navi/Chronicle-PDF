 ...
 *   - Keycloak on http://localhost:9080 with realm `chronicle-saml`
 *   - Chronicle PDF on http://localhost:8080 with PREMIUM_KEY set and
...
test.describe("Enterprise SAML (Keycloak) — full SSO flow", () => {
  ...
  test("SAML redirect, identity in settings, real split tool run", async ({
    ...
    await page.waitForURL(/\/realms\/chronicle-saml\//, {
      timeout: 30_000,
    });
