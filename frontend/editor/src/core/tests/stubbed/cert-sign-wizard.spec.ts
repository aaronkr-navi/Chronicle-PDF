  ...
  await page.route(
    ...
    (route: Route) =>
      route.fulfill({
        json: [
          {
            alias: "Anthony Chronicle",
            source: "WINDOWS_STORE",
            subject: "CN=Anthony Chronicle",
            issuer: "CN=Anthony Chronicle",
            subjectCommonName: "Anthony Chronicle",
            issuerCommonName: "Anthony Chronicle",
            serialNumber: "abc123",
            keyAlgorithm: "RSA",
...
test.describe("CertSign tool - hardware signing (desktop)", () => {
  ...
  test("offers 'This device' and lists Windows store certificates", async ({
    ...
    ).toHaveValue(/Anthony Chronicle/, { timeout: 10_000 });
  });
});
