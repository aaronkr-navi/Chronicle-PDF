 ...
 *   STIRLING_FLAVOR=saas ./gradlew :chronicle-pdf:bootRun --args="--server.port=18083 --spring.profiles.include=dev"
...
test.describe("SaaS backend smoke", () => {
  ...
  test("CORS preflight rejects tenant wildcard origin (regression #11)", async () => {
    ...
    const r = await ctx.fetch(`${SAAS_URL}/api/v1/credits`, {
      method: "OPTIONS",
      headers: {
        Origin: "https://abandoned-tenant.ssl.chronicle-pdf.cloud",
        "Access-Control-Request-Method": "GET",
      },
    });
    expect(r.status()).toBe(403);
  ...
  test("CORS preflight accepts allowed origin", async () => {
    ...
    const r = await ctx.fetch(`${SAAS_URL}/api/v1/credits`, {
      method: "OPTIONS",
      headers: {
        Origin: "https://app.chronicle-pdf.com",
        "Access-Control-Request-Method": "GET",
      },
    });
    expect(r.status()).toBe(200);
    expect(r.headers()["access-control-allow-origin"]).toBe(
      "https://app.chronicle-pdf.com",
    );
