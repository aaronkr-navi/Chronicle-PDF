...
/**
 * Seed test for Chronicle PDF E2E tests.
...
test.describe("Chronicle PDF seed", () => {
  test("seed - app loads", async ({ page }) => {
    // Navigate to the Chronicle PDF frontend
    await page.goto("/");
    ...
    await expect(
      page
        .locator(
          '.h-screen, .mobile-layout, [data-testid="dashboard"], img[alt*="Chronicle"]',
        )
        .first(),
