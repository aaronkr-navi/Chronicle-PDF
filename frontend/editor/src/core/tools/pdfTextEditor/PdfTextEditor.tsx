...
const PdfTextEditor = ({ onComplete, onError }: BaseToolProps) => {
  ...
  const handleSaveToWorkbench = useCallback(async () => {
    ...
    try {
      ...
      const parentStub = selectors.getChronicleFileStub(sourceFileId);
      if (!parentStub) {
        console.warn(
      ...
      // Create ChronicleFile and stub for the output
      ...
      await consumeFiles([sourceFileId], ChronicleFiles, stubs);
