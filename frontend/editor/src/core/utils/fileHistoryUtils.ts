...
export function groupFilesByOriginal(
...
): Map<string, ChronicleFileStub[]> {
  const groups = new Map<string, ChronicleFileStub[]>();

  // Create a map for quick lookups
  const fileMap = new Map<string, ChronicleFileStub>();
  for (const record of ChronicleFileStubs) {
  ...
  const leafFiles = ChronicleFileStubs.filter((stub) => {
    const isParentOfOthers = ChronicleFileStubs.some(
      (otherStub) => otherStub.parentFileId === stub.id,
    );
    const isOriginalOfOthers = ChronicleFileStubs.some(
  ...
  for (const leafFile of leafFiles) {
    const lineagePath: ChronicleFileStub[] = [];
    let currentFile: ChronicleFileStub | undefined = leafFile;
    ...
    while (currentFile) {
      ...
      let nextFile: ChronicleFileStub | undefined = undefined;
 ...
 */
export function hasVersionHistory(fileStub: ChronicleFileStub): boolean {
  return !!(
