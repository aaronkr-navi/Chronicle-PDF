...
cleanup() {
  ...
  pkill -f "chronicle-pdf" 2>/dev/null || true
  echo "  Done"
}
trap cleanup EXIT
...
JAR_GLOB="$TAURI_DIR/libs/chronicle-pdf-*.jar"
...
if [ "$SKIP_BUILD" = false ] || ! ls $JAR_GLOB >/dev/null 2>&1 || [ ! -f "$JRE_JAVA" ]; then
  ...
  BUILT_JAR=$(ls -t app/core/build/libs/chronicle-pdf-*.jar 2>/dev/null | head -1 || true)
