...
INSERT INTO chronicle_pdf.pricing_policy (
...
SELECT
    ...
    CURRENT_TIMESTAMP
WHERE NOT EXISTS (
    SELECT 1 FROM chronicle_pdf.pricing_policy WHERE is_default = TRUE
);

INSERT INTO chronicle_pdf.pricing_policy_step_limit (policy_id, job_source, step_limit)
SELECT p.policy_id, src.job_source, src.step_limit
FROM chronicle_pdf.pricing_policy p
CROSS JOIN (
    VALUES
        ('WEB', 10),
        ('API', 10),
...
WHERE p.is_default = TRUE
  AND NOT EXISTS (
    SELECT 1 FROM chronicle_pdf.pricing_policy_step_limit s
...
-- ---------------------------------------------------------------------------
INSERT INTO chronicle_pdf.teams (name)
SELECT 'payg-cucumber-team'
WHERE NOT EXISTS (
    SELECT 1 FROM chronicle_pdf.teams WHERE name = 'payg-cucumber-team'
);

-- ---------------------------------------------------------------------------
-- 2. payg_team_extensions sidecar — uses the default pricing policy.
-- ---------------------------------------------------------------------------
INSERT INTO chronicle_pdf.payg_team_extensions (team_id, pricing_policy_id, stripe_customer_id)
SELECT t.team_id, NULL, NULL
FROM chronicle_pdf.teams t
WHERE t.name = 'payg-cucumber-team'
  AND NOT EXISTS (
      SELECT 1 FROM chronicle_pdf.payg_team_extensions ext WHERE ext.team_id = t.team_id
  );
...
-- ---------------------------------------------------------------------------
UPDATE chronicle_pdf.users u
SET team_id = (SELECT team_id FROM chronicle_pdf.teams WHERE name = 'payg-cucumber-team')
...
-- ---------------------------------------------------------------------------
INSERT INTO chronicle_pdf.team_memberships (
    team_id, user_id, role, invited_at, created_at, updated_at
)
SELECT t.team_id, u.user_id, 'LEADER',
       CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM chronicle_pdf.teams t, chronicle_pdf.users u
WHERE t.name = 'payg-cucumber-team'
  AND u.username = 'CUSTOM_API_USER'
  AND NOT EXISTS (
      SELECT 1 FROM chronicle_pdf.team_memberships m
...
-- ---------------------------------------------------------------------------
INSERT INTO chronicle_pdf.wallet_policy (
...
WHERE t.name = 'payg-cucumber-team'
  AND NOT EXISTS (
      SELECT 1 FROM chronicle_pdf.wallet_policy wp WHERE wp.team_id = t.team_id
  );

UPDATE chronicle_pdf.wallet_policy
SET engine = 'PAYG_SHADOW', updated_at = CURRENT_TIMESTAMP
WHERE team_id = (SELECT team_id FROM chronicle_pdf.teams WHERE name = 'payg-cucumber-team');
