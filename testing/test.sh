...
usage() {
    ...
    echo "  $0 --rerun \"Chronicle-PDF-Regression Chronicle-PDF-Security-Fat-with-login,Webpage-Accessibility-full\""
    exit 0
}
...
prepare_base_image() {
    if [ "${DOCKER_BASE_CHANGED:-false}" = "true" ]; then
        ...
        if docker build -f "$PROJECT_ROOT/docker/base/Dockerfile" \
            -t chronicle-pdf-base:local \
            "$PROJECT_ROOT/docker/base"; then
            echo "✓ Built base image locally: chronicle-pdf-base:local"
            BASE_IMAGE_ARG="--build-arg BASE_IMAGE=chronicle-pdf-base:local"
        else
...
main() {
    ...
    if should_run_test "Chronicle-PDF-Ultra-Lite" || \
       should_run_test "Webpage-Accessibility-lite" || \
       should_run_test "Chronicle-PDF-Ultra-Lite-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_ULTRA_LITE="--cache-from type=gha,scope=Chronicle-PDF-Ultra-Lite --cache-to type=gha,mode=max,scope=Chronicle-PDF-Ultra-Lite"
        else
    ...
    run_tests "Chronicle-PDF-Ultra-Lite" "./docker/embedded/compose/docker-compose-latest-ultra-lite.yml"
    ...
    if should_run_test "Chronicle-PDF-Ultra-Lite-Version-Check"; then
        start_test_timer "Chronicle-PDF-Ultra-Lite-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "Chronicle-PDF-Ultra-Lite" "http://localhost:8080"; then
            passed_tests+=("Chronicle-PDF-Ultra-Lite-Version-Check")
            echo "Version verification passed for Chronicle-PDF-Ultra-Lite"
        else
            failed_tests+=("Chronicle-PDF-Ultra-Lite-Version-Check")
            capture_failure_logs "Chronicle-PDF-Ultra-Lite-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for Chronicle-PDF-Ultra-Lite"
        fi
        stop_test_timer "Chronicle-PDF-Ultra-Lite-Version-Check"
    fi
    ...
    if should_run_test "Chronicle-PDF-Security-Fat" || \
       should_run_test "Webpage-Accessibility-full" || \
       should_run_test "Chronicle-PDF-Security-Fat-Version-Check" || \
       should_run_test "Chronicle-PDF-Security-Fat-with-login" || \
       should_run_test "Chronicle-PDF-Regression Chronicle-PDF-Security-Fat-with-login" || \
       should_run_test "chronicle-pdf-fat-Disable-Endpoints" || \
       should_run_test "Disabled-Endpoints" || \
       should_run_test "chronicle-pdf-fat-Disable-Endpoints-Version-Check"; then
        ...
        if [ -n "${ACTIONS_RUNTIME_TOKEN}" ] && { [ -n "${ACTIONS_RESULTS_URL}" ] || [ -n "${ACTIONS_CACHE_URL}" ]; }; then
            DOCKER_CACHE_ARGS_FAT="--cache-from type=gha,scope=chronicle-pdf-fat --cache-to type=gha,mode=max,scope=chronicle-pdf-fat"
        else
    ...
    run_tests "Chronicle-PDF-Security-Fat" "./docker/embedded/compose/docker-compose-latest-fat-security.yml"
    ...
    if should_run_test "Chronicle-PDF-Security-Fat-Version-Check"; then
        start_test_timer "Chronicle-PDF-Security-Fat-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "Chronicle-PDF-Security-Fat" "http://localhost:8080"; then
            passed_tests+=("Chronicle-PDF-Security-Fat-Version-Check")
            echo "Version verification passed for Chronicle-PDF-Security-Fat"
        else
            failed_tests+=("Chronicle-PDF-Security-Fat-Version-Check")
            capture_failure_logs "Chronicle-PDF-Security-Fat-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for Chronicle-PDF-Security-Fat"
        fi
        stop_test_timer "Chronicle-PDF-Security-Fat-Version-Check"
    fi
    ...
    run_tests "Chronicle-PDF-Security-Fat-with-login" "./docker/embedded/compose/test_cicd.yml"

    # Only run behave tests if the container started successfully
    if [[ " ${passed_tests[*]} " =~ "Chronicle-PDF-Security-Fat-with-login" ]]; then
        ...
        cd "testing/cucumber"
        start_test_timer "Chronicle-PDF-Regression $CONTAINER_NAME"
        ...
        if python -m behave \
            ...
            if compare_file_lists "$BEFORE_FILE" "$AFTER_FILE" "$DIFF_FILE" "$CONTAINER_NAME"; then
                ...
                passed_tests+=("Chronicle-PDF-Regression $CONTAINER_NAME")
            else
                ...
                test_failure_logs["Chronicle-PDF-Regression-Temp-Files"]="$tempfile_log"
                failed_tests+=("Chronicle-PDF-Regression-Temp-Files")
            fi
            passed_tests+=("Chronicle-PDF-Regression $CONTAINER_NAME")
        else
            gha_endgroup
            failed_tests+=("Chronicle-PDF-Regression $CONTAINER_NAME")
            ...
            test_failure_logs["Chronicle-PDF-Regression $CONTAINER_NAME"]="$cucumber_log"
        ...
        stop_test_timer "Chronicle-PDF-Regression $CONTAINER_NAME"
    fi
    ...
    # ==================================================================
    run_tests "chronicle-pdf-fat-Disable-Endpoints" "./docker/embedded/compose/docker-compose-latest-fat-endpoints-disabled.yml"
    ...
    if should_run_test "chronicle-pdf-fat-Disable-Endpoints-Version-Check"; then
        start_test_timer "chronicle-pdf-fat-Disable-Endpoints-Version-Check"
        echo "Testing version verification..."
        if verify_app_version "chronicle-pdf-fat-Disable-Endpoints" "http://localhost:8080"; then
            passed_tests+=("chronicle-pdf-fat-Disable-Endpoints-Version-Check")
            echo "Version verification passed for chronicle-pdf-fat-Disable-Endpoints"
        else
            failed_tests+=("chronicle-pdf-fat-Disable-Endpoints-Version-Check")
            capture_failure_logs "chronicle-pdf-fat-Disable-Endpoints-Version-Check" "$CURRENT_CONTAINER"
            echo "Version verification failed for chronicle-pdf-fat-Disable-Endpoints"
        fi
        stop_test_timer "chronicle-pdf-fat-Disable-Endpoints-Version-Check"
    fi
